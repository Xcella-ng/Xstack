(() => {
	...
	const menuToggleActions = () => {
		...
		_chatFriends.off().upon('click', function (e) {
			e.preventDefault();
			...
			if (typeof chatMessages === 'object')
				if (Object.keys(chatMessages).length) {
					...
					Object.keys(chatMessages.messages).forEach((chatMessage) => {
						let messages = '';
						...
						Object.keys(chatMessages.messages[chatMessage]).forEach(message => {
							const messageContent = chatMessages.messages[chatMessage][message];
							messages += `
								<div style="display: flex;flex-direction: column;max-width: ${isSmallScreen() ? '85' : '70'}%;background-color: lightgray;padding: .5rem .8rem;margin-bottom: 1rem;margin-${chatMessages.messages[chatMessage][message].type === 'from' ? 'right' : 'left'}: auto;border-radius: 1rem">
